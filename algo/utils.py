# !/usr/bin/env python 3 torch Cpu
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：R02_Multi_Decen -> utilizes
@IDE    ：PyCharm
@Author ：Ymumu
@Date   ：2021/3/1 11:25
@ Des   ：store reply buffer and network models
=================================================='''
import numpy as np
import random
import torch
...
class PolicyNetwork(nn.Module):
    ...
    def evalulate_sl(self, state, deterministic, epsilon=1e-6):
        mean, log_std = self.forward(state)
        std = log_std.exp()  # no clip in evaluation, clip affects gradients flow

        normal = Normal(0, 1)
        z = normal.sample(mean.shape)
        action_0 = torch.tanh(mean) if deterministic else torch.tanh(mean + std * z.to(device))  # TanhNormal distribution as actions; reparameterization trick
        action = self.action_range * action_0
        return action
