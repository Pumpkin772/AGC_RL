...
class SAC():
    ...
    # surpervised for the policy network(actor)
    def train_policy_sl(self,batch_size,  loss_scale = 10,gamma=0.99, soft_tau=1e-2):
        state, action, reward, next_state, done = self.replay_buffer.sample(batch_size)
        # print('sample:', state, action,  reward, done)
        state = torch.FloatTensor(state).to(device)
        ...
        action_label = action
        action_policy = self.policy_net.evalulate_sl(state, deterministic=True)

        policy_loss = (action_label - action_policy).pow(2)
        policy_loss = loss_scale*policy_loss.mean()
