...
class MASAC():
    ...
    def train(self, batch_size, reward_scale,auto_entropy, target_entropy):
        ...
        for i1 in range(self.n_agents):
            alpha_loss, q_value_loss1, q_value_loss2, policy_loss = self.model[i1].train(batch_size, reward_scale=reward_scale,  auto_entropy=auto_entropy, target_entropy=-1. * self.action_dim)
            alpha_loss_list.append(alpha_loss)
            q_value_loss1_list.append(q_value_loss1)
            q_value_loss2_list.append(q_value_loss2)
            policy_loss_list.append(policy_loss)

        # return np.sum(alpha_loss_list), np.sum(q_value_loss1_list), np.sum(q_value_loss2_list), np.sum(policy_loss_list)
        return (
            np.sum([x.cpu().item() for x in alpha_loss_list]),
            np.sum([x.cpu().item() for x in q_value_loss1_list]),
            np.sum([x.cpu().item() for x in q_value_loss2_list]),
            np.sum([x.cpu().item() for x in policy_loss_list])
                )

    def train_policy_sl(self, batch_size):
        policy_loss_list = []
        for i1 in range(self.n_agents):
            policy_loss = self.model[i1].train_policy_sl(batch_size, loss_scale=10)
            policy_loss_list.append(policy_loss)

        return np.sum(policy_loss_list)
