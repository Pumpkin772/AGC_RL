...
from algo.utils_env import obtain_action, pm_converter2, state_reward, state_reward_AGC_V2 
...
def train2(args):
    ...
    model_name = 'sac_idp_v3_3seq_5beta_NE' # states为包含10个时间步的序列, beta=100
    save_reward = -2000
    ...
    env_name = 'M10B39_V2'
    # eng.set_param(env_name, 'SolverType', 'Fixed-step', nargout=0)
    # eng.set_param(env_name, 'FixedStep', '1.0', nargout=0)  # 每步 1 秒
    ...
    for ep in range(max_episodes):
        ...
        # print("pause_time:", eng.get_param(env_name + '/Network1/pause_time', 'value'))
        # print("status:", eng.get_param(env_name, 'SimulationStatus'))
        ...
        PauseTime = 1.0
        first_PauseTime = 500.0
        StopTime = 600
        eng.set_param(env_name, 'SimulationCommand', 'stop', nargout=0)
        eng.close_system(env_name, 0, nargout=0)
        eng.load_system(env_name)

        eng.set_param(env_name+'/Network1/pause_time', 'value', str(first_PauseTime), nargout=0)
        eng.set_param(env_name, 'StopTime', str(StopTime), nargout=0)
        ...
        step_max= int(round((StopTime-first_PauseTime)/PauseTime))
        while True:
            ModelStatus = eng.get_param(env_name, 'SimulationStatus') # while true轮询，但仿真状态变为paused时暂停并进入if逻辑
            # print("pause_time:", eng.get_param(env_name + '/Network1/pause_time', 'value'))
            # print("status:", eng.get_param(env_name, 'SimulationStatus'))
            # sim_time = eng.get_param(env_name, 'SimulationTime')
            ...
            if ModelStatus == 'paused':
                ...
                states, rewards, rewardsA, rewardsF, clock = state_reward_AGC_V2(data, agent_num, counter_step,step_max,args,
                ...
                #                                                                   time.time() - t1))
                if (first_PauseTime + 1.0) > StopTime:
                    dones_list[-1] = 1.0
                    eng.set_param(env_name, 'SimulationCommand', 'Stop', nargout=0)
                    ...
                    ep_rd3 = np.sum(rewards_arr[:, 2])  # 累加 agent 3 的 reward
                    ...
                    ep_rdA3 = np.sum(rewardsA_arr[:, 2])  # 累加 agent 3 的 reward
                    ...
                    ep_rdF3 = np.sum(rewardsF_arr[:, 2])  # 累加 agent 3 的 reward
                    ...
                    loss_train.append([float(time_now), ep, ep_rd1, ep_rdA1, ep_rdF1, ep_rd2, ep_rdA2, ep_rdF2, ep_rd3, ep_rdA3, ep_rdF3])
                    print(
                    ...
                    if args.tensorboard:
                        ...
                        writer.add_scalar('Reward/train_rd3', ep_rd3, global_step=ep)
                        writer.add_scalar('Reward/train_rdA3', ep_rdA3, global_step=ep)
                        writer.add_scalar('Reward/train_rdF3', ep_rdF3, global_step=ep)
    ...
    if args.save_data:
        ...
        pd_rl_loss_train = pd.DataFrame(loss_train, columns=['time', 'step','ep_rd1','ep_rdA1','ep_rdF1','ep_rd2','ep_rdA2','ep_rdF2','ep_rd3','ep_rdA3','ep_rdF3'])
