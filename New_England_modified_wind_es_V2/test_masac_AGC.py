...
from algo.utils_env import obtain_action, pm_converter2, state_reward, state_reward2, state_reward_AGC,state_reward_test
...
def one_episode(args):
    ...
    if not args.uncontrolled:
        ...
        model_name =  'sac_idp_v3_3seq_5beta'
        ...
        if model_select == 0:
            ...
            model_path_pip = rootdir + '/models/' + model_name + '/epoch/' + str(ep) + '/'
            model_test_name = model_name + '_ep' + str(ep)
    ...
    if args.with_all_communication:
        env_index[2] = 1  # 通信线路5其实不存在，也就是没有通信线路中断
    counter_env += 1
    if counter_env == train_num:
        counter_env = 0
    flag_type, flag_value, flag_com = env_index[0], env_index[1],  env_index[2] 
    # eng.M4B11Env_init(float(flag_type), float(flag_value), float(flag_com), nargout=0)  # 两个参数为随机种子
    pause_time = 0.2
    stop_time = 15.0
    ...
    step_max = int(round(15.0/pause_time))
    while True:
        ...
        if model_status == 'paused':
            ...
            data = eng.eval('sigsOut.get(\'Sim2Python\').Values.Data')
            # obtain the state and reward
            states, rewards, rewardsA, rewardsF = state_reward_AGC(data, agent_num, counter_step,
            ...
            for agent_id in range(agent_num):
                ...
                if not args.uncontrolled:
                    ...
                    if agent_num == 1:
                        action = np.ones(8)*0.0
                    else:
                        action = np.ones(3)*0.0
                actions_save.append(action)
                actions_step.extend(action)

            dones_list.append(done)
            actions_list.append(np.array(actions_step))
            actions_step_str = pm_converter2(actions_step)
            eng.set_param(env_name+'/Genrators/Python2Sim', 'value', actions_step_str, nargout=0)
            pause_time += 0.2
            print(
    ...
    rewardsA_list = np.array(rewardsA_list)
    rewardsF_list = np.array(rewardsF_list)

    #delta_f_agent0 = [P_list[5] for P_list in P_lists]  # state[0][2] 是第一个agent的 ACE0] 是第一个agent的 Δf
    delta_f_agent0 = [states[1][4] for states in states_list]  # state[0][2] 是第一个agent的 ACE

    def smooth_curve(y, kind='cubic', points=300):
        x = np.arange(len(y))
        x_new = np.linspace(0, len(y)-1, points)
        f = interp1d(x, y, kind=kind)
        y_new = f(x_new)
        return x_new, y_new

    #Δfg
    state_0_0 = [states[0][0] for states in states_list]
    state_1_0 = [states[1][0] for states in states_list]
    state_2_0 = [states[2][0] for states in states_list]
    x0, y0 = smooth_curve(state_0_0)
    x1, y1 = smooth_curve(state_1_0)
    x2, y2 = smooth_curve(state_2_0)
    plt.figure()
    plt.plot(x0, y0, label='fg1')
    plt.plot(x1, y1, label='fg2')
    plt.plot(x2, y2, label='fg3')
    plt.xlabel('Step')
    plt.ylabel('Δfg')
    plt.title('DG Frequency Deviation')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig("D:\桌面\simulink实验\\fig\M10B39\\test_5b_Δfg.svg", format='svg', dpi=300, bbox_inches='tight')
    plt.show()
    #Δfw
    #state_0_1 = [states[0][1] for states in states_list]
    state_1_1 = [states[1][1] for states in states_list]
    state_2_1 = [states[2][1] for states in states_list]
    #x0, y0 = smooth_curve(state_0_1)
    x1, y1 = smooth_curve(state_1_1)
    x2, y2 = smooth_curve(state_2_1)
    plt.figure()
    #plt.plot(x0, y0, label='fw1')
    plt.plot(x1, y1, label='fw1')
    plt.plot(x2, y2, label='fw2')
    plt.xlabel('Step')
    plt.ylabel('Δfw')
    plt.title('WT Frequency Deviation')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig("D:\桌面\simulink实验\\fig\M10B39\\test_5b_Δfw.svg", format='svg', dpi=300, bbox_inches='tight')
    plt.show()
    #Δfes
    state_0_2 = [states[0][2] for states in states_list]
    state_1_2 = [states[1][2] for states in states_list]
    state_2_2 = [states[2][2] for states in states_list]
    x0, y0 = smooth_curve(state_0_2)
    x1, y1 = smooth_curve(state_1_2)
    x2, y2 = smooth_curve(state_2_2)
    plt.figure()
    plt.plot(x0, y0, label='fes1')
    plt.plot(x1, y1, label='fes2')
    plt.plot(x2, y2, label='fes3')
    plt.xlabel('Step')
    plt.ylabel('Δfes')
    plt.title('ES Frequency Deviation')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig("D:\桌面\simulink实验\\fig\M10B39\\test_5b_Δfes.svg", format='svg', dpi=300, bbox_inches='tight')
    plt.show()
    #ΔPtie
    state_0_3 = [states[0][3] for states in states_list]
    state_0_4 = [states[0][4] for states in states_list]
    state_2_4 = [states[2][4] for states in states_list]
    x0, y0 = smooth_curve(state_0_3)
    x1, y1 = smooth_curve(state_0_4)
    x2, y2 = smooth_curve(state_2_4)
    plt.figure()
    plt.plot(x0, y0, label='Ptie12')
    plt.plot(x1, y1, label='Ptie23')
    plt.plot(x2, y2, label='Ptie31')
    plt.xlabel('Step')
    plt.ylabel('ΔPtie')
    plt.title('Ptie Deviation')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig("D:\桌面\simulink实验\\fig\M10B39\\test_5b_ΔPtie.svg", format='svg', dpi=300, bbox_inches='tight')
    plt.show()
...
def main(args):
    ...
    for ep in range(max_episodes):
        ...
        stop_time = 20.4
        eng.set_param(env_name+'/Network1/pause_time', 'value', str(pause_time), nargout=0)
        eng.set_param(env_name, 'StopTime', str(20.5), nargout=0)
        eng.set_param(env_name, 'SimulationCommand', 'start', nargout=0)
        ...
        actions_history.append(np.zeros(8))
        step_max = int(round(20.0/pause_time))
        while True:
            model_status = eng.get_param(env_name, 'SimulationStatus')
            if model_status == 'paused':
                counter_step += 1
                ...
                # obtain the state and reward
                states, rewards, rewardsA, rewardsF = state_reward(data, agent_num, counter_step,
                                                                                       step_max, args, env_index,
                                                                                       actions_history)
                ...
                if (pause_time + 0.2) > stop_time:
                    ...
                    data_list.extend(np.array(data))
                    actions_list = np.array(actions_list)
                    h_d_list.extend(np.hstack((actions_list, np.sum(actions_list[:, 0:8:2]/4, axis=1).reshape(-1, 1),
                                               np.sum(actions_list[:, 1:8:2]/4, axis=1).reshape(-1, 1))))
                    ...
                    ep_rd_a = np.sum(rewardsA_list[0:step_max][:])
                    ep_rd_f = np.sum(rewardsF_list[0:step_max][:])

                    time_now = datetime.datetime.now().strftime("%H%M%S.%f")
                    loss_test.append([float(time_now), ep, ep_rd, ep_rd_a, ep_rd_f])
                    print(
                        'Episode: {}/{} | Episode Reward: {:.4f} ({:.4f}, {:4f}, {:4f})  '
                        '| Running Time: {:.4f}'.format(ep, max_episodes, ep_rd, ep_rd_a, ep_rd_f, 
                                                        time.time() - t1))
                    reward_ep = np.sum(rewards_list[0:step_max][:])

                    if args.tensorboard:
                        writer.add_scalar('Reward/train_rd', ep_rd, global_step=ep)
                        writer.add_scalar('Reward/train_rd_a', ep_rd_a, global_step=ep)
                        writer.add_scalar('Reward/train_rd_f', ep_rd_f, global_step=ep)
    ...
    if args.save_data:
        ...
        pd_rl_loss_test = pd.DataFrame(loss_test, columns=['time', 'step', 'loss', 'loss1', 'loss2', 'loss3', 'loss21',
                                                           'loss31'])
        data_save_path = rootdir + '/test2/' + model_test_name + '/'
        ...
        pd_data_test = pd.DataFrame(action_data_save, columns=['time', 'H1', 'D1', 'H2', 'D2', 'H3', 'D3', 'H4',
                                                               'D4', 'Hsum', 'Dsum'])
    ...
    # Plot the data
    save_name_data = rootdir + '/test2/' + model_test_name + '/' + model_test_name + '_data_test' + '.csv'
    ...
    action_name_data = rootdir + '/test2/' + model_test_name + '/' + model_test_name + '_action_data_test' + '.csv'
    action_plot = pd.read_csv(action_name_data)
    action_time_plot = action_plot.iloc[:, 0]
    action_h_plot = action_plot.iloc[:, [1, 3, 5, 7]]
    action_d_plot = action_plot.iloc[:, [2, 4, 6, 8]]
