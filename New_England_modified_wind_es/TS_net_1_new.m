function [sys,x0,str,ts,simStateCompliance] = TS_net_1_new(t,x,u,flag,YY_load,ng,busdata,linedata,Currentin_idx)
...
switch flag
  ...
  case 2
    sys=[];
  case 3
    sys=mdlOutputs(t,x,u,Yff,YY_load,m,n,busdata,linedata,Currentin_idx);
  case 4
...
sizes.NumOutputs     = 2*ng+39-ng+4;% 电压包含实轴和虚轴+联络线功率*4
...
function sys=mdlOutputs(t,x,u,Yff,YY_load,m,n,busdata,linedata,Currentin_idx)
nBus=size(busdata,1);
Y=YY_load;
t=u(1);
% ===============Fault time==================
...
Vnn = -Ynn \ (Yng * Vg);  % 非发电源节点的电压（n-ng 个）

% 3. 组装全电压向量 V_bus
V_bus = zeros(nBus,1);
V_bus(Currentin_idx) = Vg;
V_bus(setdiff(1:nBus,Currentin_idx)) = Vnn;
% 读取第 k 条支路数据
k_list = [2 6 24 25]; % 第几条线路
P_list = [];
deltaP_list=[];
Pref_list=[1.4691 0.8863 -3.0123 2.1055];
for i = 1:4
    k = k_list(i);
    f_idx = linedata(k,1);  % 起点母线编号
    t_idx = linedata(k,2);  % 终点母线编号
    R = linedata(k,3);
    X = linedata(k,4);
    B = linedata(k,5);  % total shunt susceptance
    ...
    % disp(P_list)
    P_list(end+1) = P_from;
    deltaP_list(end+1) = P_from - Pref_list(i);  % 当前功率偏差
end
P_list = P_list';
deltaP_list = deltaP_list';

%% 
sys=[Vxy;abs(Vn);deltaP_list];
